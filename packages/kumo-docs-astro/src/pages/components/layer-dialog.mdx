...
<ComponentSection>
## 标准操作
添加 `Actions` 会恰好选择一个 Kumo 主操作和一个自动生成的**关闭**按钮。在移动端，操作保持在滚动正文下方、以发丝分隔线隔开；在桌面端，它们占据固定的底部操作栏。使用方不能添加自定义控件、修改按钮尺寸或增加额外 CTA。主操作接受 `variant="primary"`（默认）或 `variant="destructive"`。
