...
<p>
  使用 <code>xAxisTickFormat</code> 自定义 x 轴标签，使用
  <code>tooltipTimestampFormat</code> 自定义标准提示框和标记提示框中的时间戳。
  两者都接收以毫秒为单位的原始时间戳，因此可以一致地应用用户选择的时区。
  未提供格式化函数时，提示框时间戳使用浏览器的语言和时区。
</p>
